...
if($_POST){
    ...
    if(empty($nama_siswa)){
        ...
        // $dt_siswa=mysqli_fetch_array($qry_get_siswa);
            $update=mysqli_query($conn,"update siswa set  NIS='".$nis."', nama='".$nama_siswa."',id_kelas='".$id_kelas."', alamat='".$alamat."', no_tlp='".$telp."'  where NISN = '".$nisn."' ") or die(mysqli_error($conn));
            if($update){
            ...
            include "koneksi.php";
            $update=mysqli_query($conn,"update siswa set  NIS='".$nis."', nama='".$nama_siswa."',id_kelas='".$id_kelas."', alamat='".$alamat."', no_tlp='".$telp."', password='".md5($password)."'  where NISN = '".$nisn."' ") or die(mysqli_error($conn));
            if($update){
                echo "<script>alert('Sukses update siswa');location.href='datasiswa.php';</script>";
            } else {
                echo "<script>alert('Gagal update siswa');location.href='ubah_siswa.php?id_siswa=".$id_siswa."';</script>";
        ...
        if(empty($nama_kelas)){
            ...
            include "koneksi.php";
                $update=mysqli_query($conn,"update kelas set  nama_kelas='".$nama_kelas."', angkatan='".$angkatan."', jurusan='".$jurusan."'  where id_kelas = '".$id."' ") or die(mysqli_error($conn));
                if($update){
        ...
        if(empty($angkatan)){
            ...
            include "koneksi.php";
                $update=mysqli_query($conn,"update spp set  angkatan='".$angkatan."', tahun='".$tahun."', nominal='".$nominal."'  where id_spp = '".$id."' ") or die(mysqli_error($conn));
                if($update){
